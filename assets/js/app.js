...
function syncSidebar() {
  ...
  /* Loop through custom layer and add only features which are in the map bounds */
  for (var key in poiLayers) {
...
$.getJSON("/mappublisher/layer", function(layers) {
  ...
  layers.forEach(function(layer){
    var newLayerHolder = L.Proj.geoJson([]);
    var newLayer = {};
...
map.on("overlayadd", function(e) {
  for(var key in poiLayers) {
    if (e.layer === poiLayers[key].placeholder) {
      markerClusters.addLayer(poiLayers[key].layer);
      syncSidebar();
    }
  }
  //markerClusters.addLayer(poiLayers[e.layer]);
...
map.on("overlayremove", function(e) {
  for(var key in poiLayers) {
    if (e.layer === poiLayers[key].placeholder) {
      markerClusters.removeLayer(poiLayers[key].layer);
      syncSidebar();
    }
  }
  if (e.layer === museumLayer) {
...
function JsonToTileLayer(layer)
{
